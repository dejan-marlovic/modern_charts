...
class _PolarPoint extends _Entity {
  ...
  void draw(CanvasRenderingContext2D ctx, double percent, bool highlight) {
    num r = lerp(oldRadius, radius, percent);
    num a = lerp(oldAngle, angle, percent);
...
class RadarChart extends Chart {
  ...
  void _calculateBoundingBoxes() {
    ...
    for (int i = 0; i < entityCount; i++) {
      ...
      for (int j = 0; j < seriesCount; j++) {
        ...
        Point<num> cp = polarToCartesian(pp.center, pp.radius, pp.angle);
        minX = min(minX, cp.x);
        minY = min(minY, cp.y);
        maxX = max(maxX, cp.x);
        maxY = max(maxY, cp.y);
  ...
  void _calculateDrawingSizes() {
    ...
    num factor = 1 + sin((_xLabels.length >> 1) * _angleInterval - _PI_2);
    ...
    if (yInterval == null) {
      num yMinInterval = _options['yAxis']['minInterval'];
  ...
  void _drawAxesAndGrid() {
    int xLabelCount = _xLabels.length;
    int yLabelCount = _yLabels.length;
    ...
    if (lineWidth > 0) {
      _axesContext
        ...
        ..beginPath();
      num radius = _radius;
      for (int i = yLabelCount - 1; i >= 1; i--) {
        num angle = -_PI_2 + _angleInterval;
        _axesContext.moveTo(_center.x, _center.y - radius);
        for (int j = 0; j < xLabelCount; j++) {
          Point<num> point = polarToCartesian(_center, radius, angle);
    ...
    if (lineWidth > 0) {
      ...
      for (int i = 0; i < xLabelCount; i++) {
        Point<num> point = polarToCartesian(_center, _radius, angle);
        _axesContext
    ...
    num x = _center.x - _axisLabelMargin;
    num y = _center.y - _yLabelHop;
    _axesContext
      ..fillStyle = style['color']
      ..font = _getFont(style)
      ..textAlign = 'right'
      ..textBaseline = 'middle';
    for (int i = 1; i <= yLabelCount - 2; i++) {
    ...
    num angle = -_PI_2;
    num radius = _radius + _axisLabelMargin;
    for (int i = 0; i < xLabelCount; i++) {
  ...
  void _drawText(CanvasRenderingContext2D ctx, String text, num radius,
    ...
    num w = ctx.measureText(text).width;
  ...
  bool _drawSeries(double percent) {
    ...
    int pointCount = _xLabels.length;
    ...
    for (int i = 0; i < _seriesList.length; i++) {
      ...
      for (int j = 0; j < pointCount; j++) {
        ...
        // TODO: Optimize.
        num radius = lerp(point.oldRadius, point.radius, percent);
        num angle = lerp(point.oldAngle, point.angle, percent);
        Point<num> p = polarToCartesian(_center, radius, angle);
        if (j > 0) {
  ...
  int _getEntityGroupIndex(num x, num y) {
    ...
    for (int i = points.length - 1; i >= 0; i--) {
      num delta = angle - points[i].angle;
  ...
  Point _getTooltipPosition() {
    ...
    num offset = _options['series']['markers']['size'] * 2 + 5;
    num x = box.right + offset;
    num y = box.top + (box.height - _tooltip.offsetHeight) ~/ 2;
  ...
  _Entity _createEntity(int seriesIndex, int entityIndex, value, String color,
    ...
    num angle = _getAngle(entityIndex);
